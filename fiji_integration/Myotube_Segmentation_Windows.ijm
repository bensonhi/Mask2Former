...
function installDependenciesQuiet() {
    ...
    if (startsWith(getInfo("os.name"), "Windows")) {
        ...
        batch_content = batch_content + "echo Installing detectron2 0.6+pt1.11.0cpu (compatible with PyTorch 1.10.2)... >> \"" + install_output_file + "\"\r\n";
        batch_content = batch_content + PYTHON_COMMAND + " -m pip install \"detectron2==0.6+pt1.11.0cpu\" --find-links https://miropsota.github.io/torch_packages_builder/detectron2/ --no-deps --no-cache-dir >> \"" + install_output_file + "\" 2>&1\r\n";
        batch_content = batch_content + "echo Installing detectron2 dependencies... >> \"" + install_output_file + "\"\r\n";
        batch_content = batch_content + PYTHON_COMMAND + " -m pip install fvcore omegaconf hydra-core black yacs tabulate cloudpickle termcolor tensorboard iopath >> \"" + install_output_file + "\" 2>&1\r\n";
