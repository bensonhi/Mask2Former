...
function segmentMyotubesWithGUI() {
    ...
    // Check for success/error in the standard output directory
    // The GUI will have set the output directory, we need to check if it exists
    // For now, check in our TEMP_DIR
    success_file = OUTPUT_DIR + File.separator + "BATCH_SUCCESS";
    error_file = OUTPUT_DIR + File.separator + "ERROR";
...
function installDependencies() {
    ...
    requirements_locations = newArray(
        ...
        MASK2FORMER_PATH + File.separator + "fiji_integration" + File.separator + "requirements.txt"
    );
